...
// Configuration
const config = {
  inputDir: path.join(__dirname, "../app/views/titan-mvp-1/create-new-form"),
  prefixMappings: {
    // Route path mappings
...
function changeUrlPrefixes() {
  try {
    // Find all HTML files in the create-new-form directory and its subdirectories
    ...
    files.forEach((file) => {
      ...
      for (const [oldPrefix, newPrefix] of Object.entries(
        ...
        const pattern = new RegExp(
          `(["']|\\()${oldPrefix}|{%\\s*extends\\s*["']${oldPrefix}|{%\\s*include\\s*["']${oldPrefix}`,
          "g"
        );
        modifiedContent = modifiedContent.replace(pattern, (match) => {
          if (match.startsWith("{%")) {
            return match.includes("extends")
              ? `{% extends "${newPrefix}`
              : `{% include "${newPrefix}`;
          }
