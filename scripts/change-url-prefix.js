...
// Configuration
const config = {
  inputDir: path.join(__dirname, "../app/routes.js"),
  outputDir: path.join(__dirname, "../app/routes.js"),
  prefixMappings: {
    // Route path mappings
...
function changeUrlPrefixes() {
  try {
    // Read the input file
    let content = fs.readFileSync(config.inputDir, "utf8");
    ...
    const backupFile = config.inputDir + ".backup";
    fs.copyFileSync(config.inputDir, backupFile);
    console.log("Created backup file:", backupFile);

    // Write the modified content back to the file
    fs.writeFileSync(config.outputDir, modifiedContent, "utf8");
    console.log("Modified file: routes.js");
